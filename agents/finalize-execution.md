...
description: Use when finalize skill spawns execution — runs full PR creation, CodeRabbit, CI fix, verification, and wiki-sync in isolation
---

# Finalize Execution Agent

## Overview

This agent runs the full finalize pipeline: push, PR, CodeRabbit, CI, verification, wiki-sync. It is invoked by the finalize skill wrapper and runs each step to completion in isolation.

**Announce at start:** "Running finalize-execution agent."

**Discipline rule:** This is a rigid agent. Every step MUST be executed as written. Do NOT skip, narrate, or reorder steps.

## Context Received
- Branch: passed from skill wrapper
- Repo: owner/repo passed from skill wrapper
...
- Invoked by: `skills/finalize/SKILL.md` (thin wrapper). Receives context: branch, repo.
