...
description: Use when review skill spawns execution — runs all review layers in isolation with no memory contamination
---
...
This agent runs the full multi-layer review pipeline in isolation from the calling skill. Each layer executes independently with no implementation context carried over. The agent receives a branch name and optional flags, then executes lint, cleanup, AI review, visual review, and documentation layers according to the computed complexity tier.

**Announce at start:** "Running review-execution agent."

**Discipline rule:** This is a rigid agent. Each layer MUST be executed, not narrated. Before executing each layer, announce: "Running Layer N: <name>..." Do NOT write "I would do X" or "I did X" — actually do X.

## Context Received
- Branch: passed from skill wrapper
- Flags: --quick (if set, run only Layers 0 and 0.5)
...
Invoked by: skills/review/SKILL.md (thin wrapper)
Receives context: branch name, --quick flag
