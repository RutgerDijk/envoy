...
Review-thread fetching paginates (`pageInfo.hasNextPage`), so counts are correct past 100 threads. All four conversation-completion gates route through this snapshot instead of REST comment counts — `finalize` (`steps/coderabbit.md`), `verification`, `coderabbit-pr-review` (the resolution-verify count gate), and `finalize` (`steps/verify.md`). Count gates read `.threads.unresolved` (all-author) or `.coderabbit.unresolvedThreads` (CodeRabbit-only); queries that need comment bodies or thread node IDs stay separate, since the snapshot exposes counts, not those. REST comment counts miss inline review threads and are fooled by rate-limiting — the CodeRabbit-poll race the single source fixes.
