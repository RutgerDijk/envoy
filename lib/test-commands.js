...
function buildFilteredCommand(filteredCommand, testName) {
    ...
    // Escaping strategy is decided once, from the FIRST occurrence's
    // surrounding characters, and applied to every occurrence — templates
    // with more than one placeholder are expected to quote them
    // consistently.
    const escaped = escapeForPosition(filteredCommand, index, String(testName));
    return filteredCommand.split(PLACEHOLDER).join(escaped);
}
