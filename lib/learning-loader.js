 ...
 *
 * Used by pickup (implementation reminders) and review
...
function loadConfirmedPatterns(stackNames) {
  ...
  if (coderabbitData && coderabbitData.patterns) {
    for (const [, pattern] of Object.entries(coderabbitData.patterns)) {
      if (pattern.level === 'confirmed' || pattern.level === 'automated') {
        const patternStacks = pattern.stacks || [pattern.stack || 'general'];
        const matchesStack = !stackNames || stackNames.length === 0 ||
          patternStacks.some(s => stackNames.includes(s)) || patternStacks.includes('general');
        if (matchesStack) {
