...
function warnOnProbeFailure(err, ruleName) {
    const timedOut = err && (err.code === 'ETIMEDOUT' || err.killed === true);
    if (timedOut) {
        console.warn(
        ...
        try {
            if (rule.type === 'file') {
                ...
                const result = execSync(
                    `find "${projectDir}" -maxdepth 3 ${FIND_PRUNE} ${findFlag} "${findPattern}" -print 2>/dev/null | head -1`,
                ...
                const result = execSync(
                    `grep -r -l -E "${rule.pattern}" ${filePatterns} ${GREP_EXCLUDES} "${projectDir}" 2>/dev/null | head -1`,
                    { encoding: 'utf8', timeout: PROBE_TIMEOUT_MS }
                ).trim();
