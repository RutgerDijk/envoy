 ...
 *
 * Fetches `remoteRef` (e.g. `git fetch origin main`) before computing, so a
 * stale local remote-tracking ref doesn't silently under-report drift. If
 * the fetch fails (e.g. no network), proceeds with whatever local ref state
 * exists rather than throwing, and sets `fetchFailed: true` on the result.
 *
 * @param {string} cwd - repo working directory
 * @param {string} [remoteRef='origin/main'] - the ref to check drift against
 * @returns {{hasDrift: boolean, driftCommits: number, overlappingFiles: string[], message: string, fetchFailed: boolean}}
 */
function computeDrift(cwd, remoteRef = 'origin/main') {
  // Fetch first — the local origin/main tracking ref can be stale (session
  // open a while, teammate merged recently), which would otherwise cause
  // this check to silently under-report drift. Tolerate fetch failure (e.g.
  // no network) by proceeding with whatever local ref state exists, and
  // surface that fact via fetchFailed rather than throwing/aborting.
  const [remoteName, ...branchParts] = remoteRef.split('/');
  const remoteBranch = branchParts.join('/') || 'main';
  let fetchFailed = false;
  try {
    execFileSync(
      'git',
      ['fetch', remoteName, `+${remoteBranch}:refs/remotes/${remoteName}/${remoteBranch}`, '--quiet'],
  ...
  if (!branchPoint) {
    ...
    return { ...evaluateDrift(0, []), fetchFailed };
  }
  ...
  return { ...evaluateDrift(driftCommits, overlappingFiles), fetchFailed };
}
