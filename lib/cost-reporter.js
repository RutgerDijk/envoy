  ...
  try {
    ...
    for (const line of lines) {
      ...
      if (!usage.models[model]) {
        usage.models[model] = { input: 0, output: 0, cacheWrite: 0, cacheRead: 0, turns: 0 };
      }
      usage.models[model].input += u.input_tokens || 0;
      usage.models[model].output += u.output_tokens || 0;
      usage.models[model].cacheWrite += u.cache_creation_input_tokens || 0;
      usage.models[model].cacheRead += u.cache_read_input_tokens || 0;
      usage.models[model].turns += 1;
...
function getRecentUsage(projectDir, options = {}) {
  ...
  for (const session of sessions) {
    ...
    for (const [model, modelUsage] of Object.entries(usage.models)) {
      ...
      const pricing = findPricing(model);
      if (pricing) {
        result.byModel[model].costUSD +=
          modelUsage.input * pricing.input +
          modelUsage.output * pricing.output +
          modelUsage.cacheWrite * pricing.cacheWrite +
          modelUsage.cacheRead * pricing.cacheRead;
      }
    }
  }
 ...
 * @property {Object<string, {input: number, output: number, cacheWrite: number, cacheRead: number, turns: number}>} models
