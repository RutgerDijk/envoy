 ...
 * Looks for skill invocations, agent dispatches, and hook references.
 *
 * @param {object} msg - The assistant message object
 * @returns {string} Activity label (e.g., "skill:finishing-branch", "agent:Explore", "general")
 */
function detectActivity(msg) {
  const content = JSON.stringify(msg.content || '');

  // Skill invocations (envoy:skill-name pattern)
  const skillMatch = content.match(/envoy:([\w-]+)/);
  if (skillMatch) return 'skill:' + skillMatch[1];

  // Agent dispatches (subagent_type in tool calls)
  const agentMatch = content.match(/subagent_type.*?"([\w-]+)"/);
  if (agentMatch) return 'agent:' + agentMatch[1];

  // Agent tool usage without explicit type
  if (content.includes('"Agent"') || content.includes('Agent(')) return 'agent:general';

  // Review-related
  if (content.includes('code-reviewer') || content.includes('CodeRabbit')) return 'review';
