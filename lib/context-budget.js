...
function fitToBudget(params, tier) {
  ...
  const p = { ...params };
  const trimmed = [];
  const constraintLines = p.constraints.split('\n').length;
 ...
 * the prompt to that file (relative paths resolve against params.json's
 * directory; parent dirs are created) and prints only the verdict. When a section was
...
function cli(argv) {
  ...
  const outFile = outArg ? path.resolve(path.dirname(path.resolve(paramsFile)), outArg) : null;
  if (outFile) {
    try {
