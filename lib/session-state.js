...
function save(state, dir) {
  ...
  // Unique temp name: a fixed `.tmp` suffix races concurrent writers.
...
/**
 * Record which model every parallel implementer agent should use for the
 * remainder of this pickup run (Step 12.5: worker-model selection —
 * fable/opus/sonnet/haiku, or kimi via lib/model-dispatch.js). Chosen once
 * per pickup and persisted so a session resumed after compaction or
 * restart reads it back instead of re-asking.
 * @param {SessionState} state
 * @param {string} model - one of model-dispatch's ANTHROPIC_TIERS or KIMI_TIER
 */
function setWorkerModel(state, model) {
...
function formatForPrompt(state) {
  ...
  if (state.workerModel) {
    lines.push(`**Worker model:** ${state.workerModel} (already chosen — do not re-ask)`);
  }
 ...
 * @property {string|null} workerModel - Worker model chosen for parallel implementer dispatch (Step 12.5: fable/opus/sonnet/haiku/kimi); null until chosen.
 */
