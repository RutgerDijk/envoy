...
function validateValue(schema, value, errors, pathStr) {
  ...
  // Minimal stand-in for JSON Schema's `contains` keyword: assert that an
  // array of {name, ...} objects includes an item for each required name.
  // Used to make e.g. "the handoff must record a tests layer" a real,
  // mechanically-checked contract rather than a prose convention.
  if (schema.type === 'array' && typeOf(value) === 'array' && Array.isArray(schema.requiredItemNames)) {
    const presentNames = new Set(value.filter(v => typeOf(v) === 'object').map(v => v.name));
    for (const required of schema.requiredItemNames) {
      if (!presentNames.has(required)) {
        errors.push(`${pathStr || '<root>'}: missing required item with name "${required}"`);
      }
    }
  }
}
