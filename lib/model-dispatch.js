 ...
 * @param {string} allowedTools the normalized tool list the worker is granted
 * @param {{cwd?: string, contractPath?: string|null}} options
 */
function enforceWorkerContract(prompt, taskId, allowedTools, options) {
  ...
  const decision = evaluateWorkerPrompt(contractPath, prompt, taskId, allowedTools);
  if (decision.block) {
    throw new Error(
...
function buildKimiDispatch({ prompt, taskId, allowedTools, timeoutSeconds, maxBudgetUsd }, options = {}) {
  ...
  enforceWorkerContract(prompt, taskId, tools, options);
