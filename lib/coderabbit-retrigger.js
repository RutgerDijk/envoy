...
/**
 * Path to babysit's persistent per-PR retrigger-count state file.
 *
 * Unlike finalize (a single run, capped in-memory per invocation),
 * babysit is meant to be re-run repeatedly across `/loop` iterations —
 * each invocation is a fresh process, so the 2-retrigger cap must be
 * read from and written to disk to have any effect across passes.
 *
 * @param {string} cwd
 * @returns {string}
 */
function retriggerCountsPath(cwd) {
  return path.join(cwd, '.envoy', 'babysit', 'retrigger-counts.json');
}

/**
 * Load the persisted re-trigger count for a PR (babysit, cross-invocation).
 * Missing or corrupt state is treated as 0 — fail-open, never throws.
 *
 * @param {string} cwd - repo root (or any base dir) containing `.envoy/`
 * @param {number|string} prNumber
 * @returns {number}
 */
function loadRetriggerCount(cwd, prNumber) {
  try {
    const raw = fs.readFileSync(retriggerCountsPath(cwd), 'utf8');
    const data = JSON.parse(raw);
    const entry = data[String(prNumber)];
...
/**
 * Persist the re-trigger count for a PR (babysit, cross-invocation).
 * Merges into the existing state file rather than clobbering other PRs.
 *
 * @param {string} cwd - repo root (or any base dir) containing `.envoy/`
 * @param {number|string} prNumber
 * @param {number} count
 */
function saveRetriggerCount(cwd, prNumber, count) {
  const filePath = retriggerCountsPath(cwd);
  let data = {};
  try {
  ...
  data[String(prNumber)] = { count, lastRetriggerAt: new Date().toISOString() };
