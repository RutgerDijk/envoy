 ...
 * and embeds the machine block; pickup materializes the local (gitignored)
 * `.envoy-tasks/<n>.json` from that block.
 *
 * CLI:
...
// .envoy-tasks/ is gitignored runtime state, but the writers run inside
// CONSUMER repos whose .gitignore envoy does not control — ensure the rule
// exists wherever a task file is written. Fail-soft: never block on IO.
function ensureTasksDirIgnored(cwd) {
  try {
    const ignorePath = path.join(cwd, '.gitignore');
    const content = fs.existsSync(ignorePath) ? fs.readFileSync(ignorePath, 'utf8') : '';
    if (/^\.envoy-tasks\/?$/m.test(content)) return;
    const sep = content === '' || content.endsWith('\n') ? '' : '\n';
    fs.appendFileSync(ignorePath, `${sep}.envoy-tasks/\n`);
