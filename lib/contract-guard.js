...
function isIssueCreateCommand(command) {
  ...
  if (/\bgh\s+api\b/.test(command) && /\/issues\b/.test(command) && /(-X\s*POST|--method[= ]POST)/i.test(command)) {
    return true;
  }
  return false;
}
...
function evaluateIssueCreateGuard(cwd, command) {
  ...
  if (fs.existsSync(sessionPath)) {
    ...
    return {
      ...
      unconditional: true,
      issueNumber,
      reason: `an issue was already created in this brainstorm run (#${issueNumber}) — ` +
